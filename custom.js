...
function toggleEditMode() {
    const $blocks = $('.sound-page .sound-block');
    isEditMode = !isEditMode;
    $('body').toggleClass(editClass);
    $('.tabs__mode-indicator').text(isEditMode ? 'mode: edit' : 'mode: play');
    ...
    actionWithLoading(function () {
        if (isEditMode) {
            ...
            $('.sound-page, .deck-items').selectable('enable');
            ...
            $('.sound-page, .deck-items').selectable('disable');
...
function initDraggableMain(main, single) {
    ...
    if (single === true) {
        ...
        $elements = $('.sound-page[data-page="' + main + '"] .sound-block');
    }
...
function addSavedSoundBlock(hash, pageHash, blockSource) {
    ...
    const $mainSelector = $('.sound-page[data-page="' + pageHash + '"]');

    const html = '<a class="button is-dark sound-block' +
        (blockSource ? ' ui-selected' : '') +
        (bgColor ? ' bg-' + bgColor : '') +
        (textColor ? ' text-color-' + textColor : '') +
        '" data-hash="' + hash + '"><div class="sound-block__overlay"></div>' +
...
function recreateHowl(hash, soundPath) {
    console.log('Recreating Howl:', hash, soundPath);
...
function playSound(element, maxTime, onEnd) {
    ...
    if (!block || !block.path) {
        console.error('Sound path not found for hash:', hash);
    ...
    let howl = howlDb[hash];

    stopCurrentSound();

    /*
     * If Howl does not exist, create it again.
     */
    if (!howl) {
        console.log('Howl not found. Creating:', hash);
    ...
    /*
     * If Howl is in an error state, recreate it.
     */
    if (howl.state() === 'unloaded') {
        ...
        try {
            ...
            /*
             * Remove the broken Howl and create a fresh one.
             */
            recreateHowl(hash, block.path);
...
function setDeckCounter() {
    const $deck = $('#deck');
    const $counter = $deck.find('.deck__count');
...
function saveAllData(skipNotify) {
    ...
    activeTabs.forEach(function (hash) {
        ...
        const $mainElem = $('.sound-page' + selector);
...
function initNewPageBlocks(hash, isSaved) {
    ...
    let mainHtml = '<div class="sound-page" data-page="' + hash + '">';
    if (isSaved === true) {
        mainHtml = activePages[hash].store.get('main');
    }

    document.querySelector('.app__workspace').insertAdjacentHTML('beforeend', mainHtml);
    ...
    document.querySelector('#deck > .deck__search').insertAdjacentHTML('afterend',
    ...
    const $mainSelector = $('.sound-page' + selector);
        ...
        if (!isEditMode && !e.ctrlKey) {
            if (!training.active) {
                ...
                playSound(this, trainPlayTime, function () {
                    ...
                    $trainMode.find('.training-mode__preview').html('');
...
function closeAllTabs() {
    const selector = '.tab, .sound-page, .panel-tabs, .deck-items, .deck .search';
...
function selectedBlocksAction(message, callback) {
    ...
    if (isEditMode && $selected.length > 0) {
        ...
        if ($selected.first().parent().hasClass('sound-page') || $selected.first().parent().hasClass('main')) {
...
function startIntro() {
    ...
    const steps = [{
        ...
        attachTo: {
            element: '.navigator__close-project',
            on: 'top'
        },
        buttons: [{
            action: tour.next,
...
function updateUIVisibility() {
    const $body = $('body');
    const hideProjects = config.get('hideProjects') || false;
    const hidePages = config.get('hidePages') || false;
    const hideDeck = config.get('hideDeck') || false;
    const hideEditPanel = config.get('hideEditPanel') || false;
    const hideTabs = config.get('hideTabs') || false;
    const zenMode = config.get('zenMode') || false;

    $body.toggleClass('hide-projects', hideProjects);
    $body.toggleClass('hide-pages', hidePages);
    $body.toggleClass('hide-deck', hideDeck);
    $body.toggleClass('hide-edit-panel', hideEditPanel);
    $body.toggleClass('hide-tabs', hideTabs);
    $body.toggleClass('zen-mode', zenMode);

    updateMainHeight();
}

// Set UI toggle and synchronize zenMode
function setUIToggle(type, value) {
    if (type === 'zenMode') {
        config.set('zenMode', value);
        config.set('hideProjects', value);
        config.set('hidePages', value);
        config.set('hideDeck', value);
        config.set('hideEditPanel', value);
        config.set('hideTabs', value);
    } else {
        config.set(type, value);
        if (value) {
            const allTrue =
                config.get('hideProjects') &&
                config.get('hidePages') &&
                config.get('hideDeck') &&
                config.get('hideEditPanel') &&
                config.get('hideTabs');
            config.set('zenMode', Boolean(allTrue));
        } else {
...
function setAudioOverlay(width) {
    $currentBlock.find('.sound-block__overlay').width(width);
}
...
function renderSoundFrame(frame, pageHash) {
    const selector = '.sound-page[data-page="' + pageHash + '"]';
    ...
    '<div class="sound-frame__title" style="' + titleStyle + '">' +
    '<span class="sound-frame__title-text" title="Двойной клик для переименования">' + frame.title + '</span>' +
    '<button class="button is-small sound-frame__delete" title="Удалить рамку"><i class="fa fa-times"></i></button>' +
    '</div></div>';
...
function updateFrameBounds(frameId, pageHash) {
    ...
    frame.blockHashes.forEach(function (hash) {
        const $el = $('.sound-page[data-page="' + page + '"] .sound-block[data-hash="' + hash + '"]');
        if ($el.length > 0) {
    ...
    if (minLeft !== Infinity) {
        ...
        const $element = $('.sound-page[data-page="' + page + '"] .sound-frame[data-id="' + frameId + '"]');
        $element.css({
            top: frame.rect.top,
...
function resetTrainingMode() {
    ...
    $trainMode.find('.training-mode__number').text('0');
    $trainMode.find('.training-mode__preview').html('');
    $('.is-outline').removeClass('is-outline');
...
function pickNextTrainingSound() {
    ...
    const pageSelector = '.sound-page[data-page="' + training.page + '"]';
    ...
    $trainMode.find('.training-mode__preview').html($block);
...
$(function () {
    ...
    function getSettingsMenuHtml() {
        ...
        return '<div class="panel settings-panel">' +
            ...
            '<a class="panel-block open-interface-menu"><i class="fa fa-desktop"></i> Интерфейс <i class="fa fa-chevron-right" style="margin-left: auto;"></i></a>' +
            '</div>' +
            '<div class="settings-interface-view" style="display: none;">' +
            '<p class="panel-heading"><a class="back-to-main-settings" title="Назад"><i class="fa fa-chevron-left" style="margin-right: 8px; cursor: pointer;"></i></a>Интерфейс</p>' +
            '<a class="panel-block toggle-ui" data-type="hideProjects"><i class="fa ' + (config.get('hideProjects') ? 'fa-check-square-o' : 'fa-square-o') + '"></i> Скрыть проекты</a>' +
            '<a class="panel-block toggle-ui" data-type="hidePages"><i class="fa ' + (config.get('hidePages') ? 'fa-check-square-o' : 'fa-square-o') + '"></i> Скрыть страницы</a>' +
            '<a class="panel-block toggle-ui" data-type="hideDeck"><i class="fa ' + (config.get('hideDeck') ? 'fa-check-square-o' : 'fa-square-o') + '"></i> Скрыть колоду</a>' +
    ...
    tippy(document.querySelector('#settings'), {
        ...
        placement: 'right',
        onShow: function (tip) {
            tip.setContent(getSettingsMenuHtml());
        }
    });
    ...
    $tabList.sortable({
        ...
        $main = $('.sound-page' + selector);
        $deckItems = $('.deck-items' + selector);
        updateDeckData();

        const query = '.sound-page, .panel-tabs, .deck-items, .deck .search';
    ...
    // Init page search
    pageSearch.bar = new SBar(document.querySelector('#page-search .navigator__pages-list'));
    ...
    // Init project search
    projectSearch.bar = new SBar(document.querySelector('#project-search .navigator__projects-list'));
    projectSearch.list = new List('project-search', {
        valueNames: ['text'],
        listClass: 'simplebar-content'
    });
    projectSearch.list.on('searchComplete', _.debounce(function () {
        $('#project-search .navigator__projects-list .panel-block').each(function () {
...
$body
   ...
   .keyup(function (e) {
    ...
    $body.on('click', '.quick-search__close', function () {
        closeQuickSearch();
    }).on('click', '.quick-search__clear', function () {
        $quickSearch.find('.input').val('').focus();
        $('.is-f2-found').removeClass('is-f2-found');
    });
    ...
    tippy(document.querySelector('.app__workspace'), {
        target: '.sound-block',
        theme: 'info',
        distance: 5,
        arrow: true,
    ...
    tippy(document.querySelector('.navigator__pages'), {
        target: '.panel-block',
        theme: 'info',
        arrow: true,
        placement: 'right',
